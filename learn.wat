...
(module
  ...
  (func $basic_operations
    ...
    (local $add_result f64)
    (local $mult_result f64)

    (local.set $add_result (f64.add (f64.const 2) (f64.const 4)))
  ...
  (func $get_16 (result f64)
    (f64.const 16)
  )

  (func $add (param $param0 f64) (param $param1 f64) (result f64)
    (f64.add
      (local.get $param0)
      (local.get $param1)
    )
  ...
  (func $print_args (param $arg0 f64) (param $arg1 f64)
    (call $print_f64 (local.get $arg0))
    (call $print_f64 (local.get $arg1))
  )
  ...
  ;; like (f64.mul (f64.const 8) (f64.const 2)) to the f64.store function.
  ...
  (func $apply_cos64 (param $array_length f64)
   ...
   ;;  (local $temp_64 f64)
    ...
    (local.set $idx_bytes (f64.const 0)) ;; not entirely necessary
    (local.set $bytes_per_double (f64.const 8))

    (block
      (loop
        ;; this sets idx_bytes to bytes offset of the value we're interested in.
        (local.set $idx_bytes (f64.mul (local.get $idx) (local.get $bytes_per_double)))
        ...
        (local.set $idx (f64.add (local.get $idx) (f64.const 1)))

        ;; stop the loop if the loop counter is equal the array length
        (br_if 1 (f64.eq (local.get $idx) (local.get $array_length)))
        (br 0)
      )
    )
  )
  ...
  (global $memstack_ptr (mut f64) (f64.const 65536))

  ;; Structs can only be returned by reference
  (func $sum_struct_create 
        (param $sum_struct_ptr f64) 
        (param $var$a f64) 
        (param $var$b f64)
  ...
  (func $sum_local (result f64)
    (local $var$sum_struct$a f64)
    (local $var$sum_struct$b f64)
    (local $local_memstack_ptr f64)
    ...
    local.tee $local_memstack_ptr ;; tee both stores and returns given value
    ...
    (local.set $var$sum_struct$a
      (f64.load offset=0 (local.get $local_memstack_ptr))
    )
    (local.set  $var$sum_struct$b
      (f64.load offset=4 (local.get $local_memstack_ptr))
    )
